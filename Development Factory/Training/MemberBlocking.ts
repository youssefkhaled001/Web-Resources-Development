function BlockMember(formContext: Xrm.FormContext): void {
  var width = 450;
  var height = 300;
...
function UnblockMember(formContext: Xrm.FormContext): void {
...
function BlockStatusAutomation(
...
function BlockSelected(primaryControl: Xrm.Controls.GridControl) {
  const reason = prompt(
    "Enter the reason for blocking the selected members",
    "Test"
  );
  if (!reason) return;
  Xrm.Navigation.openConfirmDialog({
    text: `Are you sure you want to block ${primaryControl
      .getGrid()
      .getSelectedRows()
      .getLength()} Selected Member${
      primaryControl.getGrid().getSelectedRows().getLength() > 1 ? "s" : ""
    }?`,
    ...
    primaryControl
      ...
      .forEach((row) => {
        ...
        const guid = row.data.entity.getId();
        const entityName = row.data.entity.getEntityName();
        const entity = {
          cr9bc_blocked: true,
        ...
        Xrm.WebApi.updateRecord(entityName, guid, entity).then(()=>{
...
function UnblockSelectedMembers(primaryControl: Xrm.Controls.GridControl){
  Xrm.Navigation.openConfirmDialog({
